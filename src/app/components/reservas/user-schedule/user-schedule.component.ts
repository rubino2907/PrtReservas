...
export class UserScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadReservesByCreatedByFromCookies(); // Carrega as reservas ao inicializar o componente
  ...
  previous(): void {
    ...
    loadReservesByCreatedByFromCookies(): void {
      // Obter o createdBy do cookie, supondo que esteja armazenado em 'username'
      const createdBy = this.cookieService.get('userName');
      console.log('Criado por (userName):', createdBy); // Log para mostrar o createdBy
      
      // Verificar se o createdBy foi obtido
      if (!createdBy) {
        console.error('Não foi possível obter o createdBy dos cookies.');
        return;
      }
    
      console.log('Carregando reservas para o createdBy:', createdBy); // Log para mostrar que as reservas estão sendo carregadas
      
      // Limpar os eventos antes de carregar as reservas do novo usuário
      this.events = [];
    
      // Chamar o serviço para buscar as reservas pelo createdBy
      this.reserveService.getReservesByCreatedBy(createdBy)
        .subscribe((reserves: Reserve[]) => {
          console.log('Reservas encontradas:', reserves); // Log para mostrar as reservas encontradas
    
          // Adicionar as reservas aos eventos
          reserves.forEach(reserve => {
            if (reserve.dateStart && reserve.dateEnd) {
              this.events.push({
                start: new Date(reserve.dateStart),
          ...
          console.log('Eventos atualizados com reservas:', this.events); // Log para mostrar os eventos atualizados com as reservas
        });
    }
